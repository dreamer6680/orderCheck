...
import type { InventoryProjection, OrderResponse, ProductResponse } from '@/lib/api/generated'
...
export default function Page() {
  ...
  const recentOrders = useMemo(() => orders.map((order) => {
    const meta = statusMeta[order.status ?? 'PENDING_CHECK'] ?? statusMeta.PENDING_CHECK
    return {
      no: order.orderNo ?? '-',
      customer: order.customerName ?? '-',
      items: (order.items ?? []).map((item) => `${item.productName ?? item.sku ?? '商品'} × ${item.orderedQuantity ?? 0}`).join('、'),
      status: meta.label,
      type: meta.type,
  ...
  const inventory = useMemo(() => {
    ...
    return inventoryData.map((item) => {
      ...
      const status = safety > 0 && available < safety * 0.5 ? '需补货' : safety > 0 && available < safety ? '低库存' : '充足'
      return {
    ...
    }).sort((a, b) => (a.status === '充足' ? 1 : -1) - (b.status === '充足' ? 1 : -1)).slice(0, 5)
  }, [inventoryData, products])

  const filteredOrders = useMemo(() => recentOrders.filter((order) => `${order.no}${order.customer}${order.items}`.toLowerCase().includes(search.toLowerCase())), [recentOrders, search])
  const pendingCheckCount = orders.filter((order) => order.status === 'PENDING_CHECK').length
  const pendingOutboundCount = orders.filter((order) => order.status === 'PENDING_OUTBOUND').length
  const abnormalCount = orders.filter((order) => order.status === 'ABNORMAL').length
  const lowStockCount = inventoryData.filter((item) => {
    const safety = products.find((product) => product.id === item.productId)?.safetyStock ?? 0
